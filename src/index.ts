...
// Exit codes considerados retriaveis: 1 (error), 2 (blocked), 3 (inconclusive)
...
export async function main(argv: string[] = process.argv): Promise<number> {
  const config: Config = parseCli(argv.slice(2));

  // --merge-history: emit deduped union of recent searches (no browser)
  ...
  // Cache lookup: history-backed reuse para mesma query dentro do TTL
  if (config.useCache) {
  ...
  // Lock per-port: serializa execucoes concorrentes que compartilhariam
  // o mesmo Chrome/CDP e teriam keystrokes intercaladas.
